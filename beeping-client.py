...
def send_data_graphite(schema, backend_addr, backend_port, pmb_return):
    g = graphitesend.init(graphite_server=backend_addr,
                          ...
                          system_name='', prefix='')
    for i in pmb_return:
        if type(pmb_return[i]) == int: g.send(schema+"."+i, pmb_return[i])

#Function to send data to influxdb
def send_data_influxdb(backend_addr, backend_port, pmb_return, backend_user,
    ...
    client.write_points([{
        ...
        "time": current_time,
        "fields": {"value": pmb_return["http_status_code"]}
    }])
    client.write_points([{
        "measurement": "http_request_time",
        "tags": {"host": payload["url"], "region": ""},
        "time": current_time,
        "fields": {"value": pmb_return["http_request_time"]}
    }])
    client.write_points([{
        "measurement": "dns_lookup",
        "tags": {"host": payload["url"], "region": ""},
        "time": current_time,
        "fields": {"value": pmb_return["dns_lookup"]}
    }])
    client.write_points([{
        "measurement": "tcp_connection",
        "tags": {"host": payload["url"], "region": ""},
        "time": current_time,
        "fields": {"value": pmb_return["tcp_connection"]}
    }])
    client.write_points([{
        "measurement": "server_processing",
        "tags": {"host": payload["url"], "region": ""},
        "time": current_time,
        "fields": {"value": pmb_return["server_processing"]}
    }])
    client.write_points([{
        "measurement": "content_transfer",
        "tags": {"host": payload["url"], "region": ""},
        "time": current_time,
        "fields": {"value": pmb_return["content_transfer"]}
    }])
    ...
    if pmb_return["ssl"] == True:
        client.write_points([{
            ...
            "tags": {"host": payload["url"], "region": ""},
            "time": current_time,
            "fields": {"value": pmb_return["tls_handshake"]}
        }])
        client.write_points([{
            "measurement": "ssl_days_left",
            "tags": {"host": payload["url"], "region": ""},
            "time": current_time,
            "fields": {"value": pmb_return["ssl_days_left"]}
        }])
...
parser.add_argument('-s',
                    ...
                    default="test.test.prod.host.pingmeback")
...
if backend == "influxdb":
    send_data_influxdb(backend_addr, backend_port, pmb_return, backend_user,
                       bakend_pwd, backend_db)
